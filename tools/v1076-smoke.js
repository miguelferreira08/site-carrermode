...
ok(/\?v=10\.7\.6/.test(html),'cache buster updated to v10.7.6');
...
ok(/font-family:'BTL Unisportion'/.test(css)&&/UT Unisportion FC Demo/.test(css),'exact uploaded Unisportion Demo family is mapped');
ok(/UT%20Unisportion%20FC%20Demo\.ttf/.test(css),'webfont path is wired for the user-supplied font');
ok(/html,body,body \*,button,input,textarea,select,option,optgroup/.test(css)&&/--btl-display-font:'BTL Unisportion'/.test(css),'Unisportion family is globally forced in CSS');

const ids=[...html.matchAll(/\bid="([^"]+)"/g)].map(m=>m[1]);
const dup=[...new Set(ids.filter((x,i)=>ids.indexOf(x)!==i))];
ok(dup.length===0,'HTML has no duplicate IDs');
console.log(`\n${passed} v10.7.6 checks passed.`);
